...
public class MultiGameUI : CSingletonMonobehaviour<MultiGameUI>
{
    ...
    private void Awake()
    {
        this.sendManager = GameObject.Find("MultiGameManager").GetComponent<MultiSendManager>();
        this.waiting_packets = new Queue<List<string>>();
        this.card_collision_manager = GameObject.Find("MultiGameManager").GetComponent<CardCollision>();
        this.card_collision_manager.callback_on_touch = this.on_card_touch;

        this.player_me_index = 0;
        this.ui_slot_index = 0;

        this.game_info = new MultiGameInfo();
    ...
    //Start함수에서 multimanager에서 호출하는 함수로 변경
    public void ui_start()
    {
        StartCoroutine(CoroutineUtil.RunThrowingIterator(PaketHandler()));
    ...
    IEnumerator PaketHandler()
    {
        while (true)
        {
            ...
            switch (protocol)
            {
                case 1:
                    {
                        Debug.Log("READY_TO_SERVER_START");

                        sendManager.local_server_start();
                    }
                    break;
                ...
                case 2:
                    {
                        ...
                        if (player_me_index == 1)
                        {
                            List<string> msg = new List<string>();
                            msg.Add((short)PROTOCOL.SET_START_USER + "");
                            MultiSendManager.instance.send_to_game_room(player_me_index, msg);
                        }
                    }
                    break;
                ...
                case 11:
                    {
                        ...
                        MultiPlayManager.instance.game_playing = true;
                        ...
                        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(distribute_cards(player_cards)));
                    }
                    break;
                ...
                case 33:
                    {
                        ...
                        for (byte i = 0; i < bonus_card_count; ++i)
                        {
                            ...
                            yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(flip_deck_card(player_index, card_number_msg, card_pae_type, card_position)));
                        }
                ...
                case 14:
                    {
                        ...
                        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(on_select_card_ack(msg_list)));
                    }
                    break;
                ...
                case 19:
                    {
                        ...
                        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(on_flip_deck_card_ack(msg_list)));
                    }
                    break;
                ...
                case 46:
                    {
                        ...
                        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(on_flip_deck_bonus_card_ack(player_cards)));
                    }
                    break;
                ...
                case 55:
                    {
                        ...
                        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(on_flip_plus_bonus_card_ack(msg_list)));
                    }
                    break;
                ...
                case 57:
                    {
                        ...
                        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(move_bomb_flip_bonus_card(player_index, deck_card_number, deck_card_pae_type, deck_card_position)));
                    }
                    break;
                ...
                case 20:
                    {
                        ...
                        byte player_index = (byte)Int32.Parse(PopAt(msg_list));
                        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(on_turn_result(player_index, msg_list)));
                    }
                    break;
                ...
                case 21:
                    {
                        ...
                        else
                        {
                            ...
                            popup.on_popup(1, player_me_index);
                        }
                    }
                    break;
                ...
                case 26:
                    {
                        ...
                        byte player_index = (byte)Int32.Parse(PopAt(msg_list));
                        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(move_kookjin_to_pee(player_index)));
                    }
                    break;
                ...
                case 27:
                    {
                        ...
                        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(game_over(winner, winner_score, winner_go, winner_ppuck, winner_shaking, winner_kwang, winner_pee, winner_yeol, winner_tee,
                ...
                case 82:
                    {
                        Debug.Log("ON_SET_START_USER");
                        //카드를 선택하는 팝업창을 띄운다.
                        PopupManager.instance.show(UI_PAGE.POPUP_SET_START_USER);
                        PopupFirstUser popup = PopupManager.instance.get_uipage(UI_PAGE.POPUP_SET_START_USER).GetComponent<PopupFirstUser>();

                        Queue<Card> cards = new Queue<Card>();
                        for (int i = 0; i < 3; i++)
                        {
                            byte number = (byte)Convert.ToInt32(PopAt(msg_list));
                            PAE_TYPE type = Converter.PaeType(PopAt(msg_list));
                        ...
                        popup.refresh(player_me_index, cards);
                    }
                    break;

                case 84:
                    {
                        Debug.Log("START_USER_SELET_START");

                        PopupFirstUser popup = PopupManager.instance.get_uipage(UI_PAGE.POPUP_SET_START_USER).GetComponent<PopupFirstUser>();
                        byte player = Convert.ToByte(PopAt(msg_list));
                        popup.touch_on_turn(player);
                    }
                    break;
                ...
                case 87:
                    {
                        //플레이어가 선택한 결과를 보여준다
                        Debug.Log("SET_START_USER_CARD_ACK");

                        PopupFirstUser popup = PopupManager.instance.get_uipage(UI_PAGE.POPUP_SET_START_USER).GetComponent<PopupFirstUser>();
                        byte player = Convert.ToByte(PopAt(msg_list));
                        int pick_slot = Convert.ToInt32(PopAt(msg_list));

                        popup.show_pick_card(player, pick_slot);
                    }
                    break;

                case 89:
                    {
                        Debug.Log("END_SET_START_USER");
                        PopupFirstUser popup = PopupManager.instance.get_uipage(UI_PAGE.POPUP_SET_START_USER).GetComponent<PopupFirstUser>();

                        int result = (byte)Int32.Parse(PopAt(msg_list));

                        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(popup.ShowCardResult(result)));
                    }
                    break;

                case 900:
                    {
                        MultiPlayManager.instance.on_system_message("상대방이 나가기 예약을 하였습니다.");
                    }
                    break;

                case 901:
                    {
                        MultiPlayManager.instance.on_system_message("상대방이 나가기 예약을 취소하였습니다.");
                    }
                    break;

                case 990:
                    {
                        MultiPlayManager.instance.other_pocus_out();
                    }
                    break;

                case 991:
                    {
                        MultiPlayManager.instance.other_pocus_on();
                    }
                    break;

                case 999:
                    {
                        MultiPlayManager.instance.on_other_out_popup();
                    }
                    break;
            }
        }
    ...
    public Sprite get_hwatoo_sprite(Card card)
    {
    ...
    IEnumerator flip_deck_card(byte player_index, byte card_number, PAE_TYPE card_pae_type, byte card_position)
    {
        ...
        if (card_number_msg == 12)
        {
            ...
            yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(CoroutineUtil.RunThrowingIterator(move_flip_start_event_card_on_player(player_index, card_number_msg, card_pae_type_msg, card_position_msg))));
        }
        else
        {
            yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(CoroutineUtil.RunThrowingIterator(move_flip_start_event_card(card_number_msg, card_pae_type_msg, card_position_msg))));
        }
    }
    ...
    IEnumerator move_flip_start_event_card_on_player(byte player_index, byte number, PAE_TYPE pae_type, byte position)
    {
        ...
        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(CoroutineUtil.RunThrowingIterator(flip_deck_card_ani(deck_card_picture))));
    ...
    IEnumerator move_flip_start_event_card(byte number, PAE_TYPE pae_type, byte position)
    {
        ...
        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(flip_deck_card_ani(deck_card_picture)));
    ...
    IEnumerator game_over(byte winner, byte winner_score, byte winner_go, byte winner_ppuck, byte winner_shaking, byte winner_kwang, byte winner_pee, byte winner_yeol, byte winner_tee,
        byte losser_go, byte losser_kwang, byte losser_pee, byte losser_yeol, byte losser_tee, byte winner_start_ppuck, byte losser_start_ppuck, byte nagari)
    {
       ...
       " winner_kwang" + winner_kwang + " winner_pee" + winner_pee + " winner_yeol" + winner_yeol + " winner_tee" + winner_tee);
        ...
        MultiPlayManager.instance.game_playing = false;
        ...
        if (winner_ppuck > 2)
        {
            ...
            if (money_value < 0)
            {
                money = "-" + MoneyManager.instance.convert_money_to_string(-money_value);
            }
            else
            {
                money = MoneyManager.instance.convert_money_to_string(money_value);
            ...
            if (winner == player_me_index)
            {
                ...
                PopupMultiGameResult popup = PopupManager.instance.get_uipage(UI_PAGE.POPUP_GAME_RESULT).GetComponent<PopupMultiGameResult>();
                popup.on_result(player_me_index, win, winner_score, 0, 0, false, false, false, false, false, winner_start_ppuck, losser_start_ppuck, nagari, double_val, final_score, money_value, over_money);
            }
            else
            {
                byte win = 0;
        ...
        else
        {
            ...
            if (winner == player_me_index)
            {
                ...
                PopupManager.instance.show(UI_PAGE.POPUP_GAME_RESULT);
                PopupMultiGameResult popup = PopupManager.instance.get_uipage(UI_PAGE.POPUP_GAME_RESULT).GetComponent<PopupMultiGameResult>();
                popup.on_result(player_me_index, win, winner_score, winner_go, winner_shaking, kwang, yeol, tee, pee, go_back, winner_start_ppuck, losser_start_ppuck, nagari, double_val, final_score, money_value, over_money);
            }
            ...
            else//무승부일 경우 다음판의 판돈을 2배로 올려 이어서 치게 된다.
            {
                ...
                PopupManager.instance.show(UI_PAGE.POPUP_GAME_RESULT);
                PopupMultiGameResult popup = PopupManager.instance.get_uipage(UI_PAGE.POPUP_GAME_RESULT).GetComponent<PopupMultiGameResult>();
                popup.on_result(player_me_index, win, 0, 0, 0, false, false, false, false, false, 0, 0, 0, 0, 0, 0, false);
            }
        }
    ...
    byte get_other_index(byte player_index)
    {
        if (player_index == 0)
        {
            return 1;
    ...
    {
        Debug.Log("on_turn_result");
        List<Card> cards_to_give = parse_cards_to_get(msg_list);
        List<CardPicture> take_cards_from_others = parse_cards_to_take_from_others(player_index, msg_list);

        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(move_after_flip_card(player_index, take_cards_from_others, cards_to_give)));
    }
    ...
    IEnumerator on_select_card_ack(List<string> msg_list)
    {
        ...
        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(move_player_cards_to_floor(player_index, card_event, bomb_cards_info, slot_index, player_card_number, player_card_pae_type, player_card_position)));
        ...
        if (player_index == this.player_me_index)
        {
            ...
            if (select_result == PLAYER_SELECT_CARD_RESULT.CHOICE_ONE_CARD_FROM_PLAYER)
            {
                ...
                else
                {
                    ...
                    popup.on_popup(1, player_me_index, select_result, target_to_choice[0], target_to_choice[1]);
                }
            }
            else
            {
    ...
    IEnumerator move_player_cards_to_floor(byte player_index, CARD_EVENT_TYPE event_type, List<Card> bomb_cards_info, byte slot_index, byte player_card_number, PAE_TYPE player_card_pae_type, byte player_card_position)
    {
        ...
        for (int i = 0; i < targets.Count; ++i)
        {
            ...
            // 스케일 장면.
            yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(scale_to(player_card, 1.0f, 0.025f)));
    ...
    public void on_select_bonus_card_ack(byte player_index_msg, byte slot_index, byte card_number, PAE_TYPE card_pae_type, byte card_position)
    {
        ...
        StartCoroutine(CoroutineUtil.RunThrowingIterator(move_player_bonus_cards_to_player_floor(player_index, slot_index, card_number, card_pae_type, card_position)));
    ...
    IEnumerator on_flip_deck_card_ack(List<string> msg_list)
    {
        ...
        Debug.Log("on_flip_deck_card_ack");
        byte player_index = (byte)Int32.Parse(PopAt(msg_list));
        ...
        if (result == PLAYER_SELECT_CARD_RESULT.CHOICE_ONE_CARD_FROM_DECK)
        {
            ...
            yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(move_flip_card(deck_card_number, deck_card_pae_type, deck_card_position)));
            ...
            if (player_index == this.player_me_index)
            {
                ...
                else
                {
                    PopupManager.instance.show(UI_PAGE.POPUP_CHOICE_CARD);
                    PopupChoiceCard popup = PopupManager.instance.get_uipage(UI_PAGE.POPUP_CHOICE_CARD).GetComponent<PopupChoiceCard>();
                    popup.on_popup(1, player_me_index, result, target_to_choice[0], target_to_choice[1]);
                }
            }
        }
        else
    ...
    IEnumerator on_flip_plus_bonus_card_ack(List<string> msg_list)
    {
        ...
        // 화면 연출 진행.
        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(move_plus_flip_bonus_card(pick_card_number, pick_card_pae_type, pick_card_position, flip_number, flip_pae_type, flip_position)));
    ...
    IEnumerator move_bomb_flip_bonus_card(byte player_index, byte number, PAE_TYPE pae_type, byte position)
    {
        ...
        StartCoroutine(CoroutineUtil.RunThrowingIterator(on_flip_bomb_bonus_card_move(player_index, deck_card_picture)));
    }
    ...
    IEnumerator move_after_flip_card(byte player_index, List<CardPicture> take_cards_from_others, List<Card> cards_to_give)
    {
        ...
        for (int i = 0; i < cards_to_give.Count; ++i)
        {
            ...
            if (this.player_me_index == player_index)
            {
                //.card.set_card_status(getCardStatus(card_pic));
                card_pic.transform.localScale = SCALE_TO_MY_FLOOR;
            }
            else
            {
    ...
    List<CardPicture> parse_cards_to_take_from_others(byte player_index, List<string> msg)
    {
        // 뺏어올 카드.
        ...
        else
        {
            ...
            else
            {
                ...
                if (same_on_hand.Count == 3 && same_on_floor == 0)
                {
                    ...
                    popup.popup_on(1, player_me_index, card_picture.card, card_picture.slot, sprites);
                }
                else
                {
                    send_select_card(card_picture.card, card_picture.slot, 0);
    ...
    Vector3 get_ui_slot_position(VisualFloorSlot slot)
    {
        ...
        position.x += (stacked_count * 16f);
        position.y -= (stacked_count * 12f);
        return position;
    }
    #endregion
    ...
    IEnumerator flip_deck_card_ani(CardPicture deck_card_picture)
    {
        ...
        yield return StartCoroutine(CoroutineUtil.RunThrowingIterator(scale_to(deck_card_picture, 1.0f, 0.025f)));
    }
    ...
    void update_player_statistics(List<string> msg)
    {
        ...
        this.game_info.set_my_score(score);
        this.game_info.set_my_go_count(go_count);
