...
{ 
    ...
    public AIGameRoom(AISendManager MultiSendManager)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            {
                case 0:
                    this.players.Add(new AIPlayer((byte)i, PLAYER_TYPE.HUMAN, MultiSendManager.send_to_ui, this));
                    break;
                case 1:
                    this.players.Add(new AIPlayer((byte)i, PLAYER_TYPE.AI, null, this));
                    break;
            }
        }
    ...
    {
        Debug.Log("AIGameRoom on_ready_req");
        ...
        this.engine.on_ai_start(this.players);
    ...
    {
        Debug.Log("AIGameRoom on_restart_req");

        reset();

        this.engine.on_ai_start(this.players); // 고스톱엔진에게 시작하게 해서 카드 셔플 및 분배
    ...
    void send_cardinfo_to_player(AIPlayer player)
    {
        Debug.Log("AIGameRoom send_cardinfo_to_player");
    ...
    public void broadcast_game_result(byte winner)//게임이 끝났을 때 네크워크에서 정보를 받아 승패를 나눔
    {
        ...
        PlayerAgent ai_player = this.players[1].agent;
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            switch (winner)
            {
                case 0:
                    ...
                    result_msg.Add(ai_player.go_count.ToString());
                    result_msg.Add(ai_player.get_kwang_count().ToString());
                    result_msg.Add(ai_player.get_pee_count().ToString());
                    result_msg.Add(ai_player.get_yeol_count().ToString());
                    result_msg.Add(ai_player.get_tee_count().ToString());

                    result_msg.Add(engine.get_nagari().ToString());

                    result_msg.Add(player.start_ppuk.ToString());
                    result_msg.Add(ai_player.start_ppuk.ToString());
                    break;

                case 1:
                    result_msg.Add(ai_player.score.ToString());
                    result_msg.Add(ai_player.go_count.ToString());
                    result_msg.Add(ai_player.ppuk_count.ToString());
                    result_msg.Add(ai_player.shaking_count.ToString());
                    result_msg.Add(ai_player.get_kwang_count().ToString());
                    result_msg.Add(ai_player.get_pee_count().ToString());
                    result_msg.Add(ai_player.get_yeol_count().ToString());
                    result_msg.Add(ai_player.get_tee_count().ToString());
